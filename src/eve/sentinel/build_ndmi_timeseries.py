...
# build_ndmi_timeseries.py -> sentinel -> eve -> src -> EVE
...
# ==============================================================
# STATISTICAL API : STATISTIQUES NDMI JOURNALIÈRES
# ==============================================================
...
def request_ndmi_statistics(
    ...
    """Récupère les statistiques NDMI journalières."""
...
def get_bin_count(
    ...
    """Récupère le nombre de pixels d'une classe NDMI."""
...
def parse_statistics(
    ...
    for interval_data in response_data.get("data", []):
        ...
        threshold_counts = {
            "ratio_ndmi_below_0": get_bin_count(
                bins,
                -1.0,
                0.0,
            ),
            "ratio_ndmi_0_0_2": get_bin_count(
                bins,
                0.0,
                0.2,
            ),
            "ratio_ndmi_0_2_0_4": get_bin_count(
                bins,
                0.2,
                0.4,
            ),
            "ratio_ndmi_0_4_0_6": get_bin_count(
                bins,
                0.4,
                0.6,
            ),
            "ratio_ndmi_0_6_0_8": get_bin_count(
                bins,
                0.6,
                0.8,
            ),
            "ratio_ndmi_above_0_8": get_bin_count(
                bins,
                0.8,
                1.0,
            ),
        ...
        rows.append(
            {
                ...
                "ndmi_min": statistics.get("min"),
                "ndmi_max": statistics.get("max"),
                "ndmi_mean": statistics.get("mean"),
                "ndmi_std": statistics.get("stDev"),

                "ndmi_p10": get_percentile(
                    percentiles,
                    10,
                ),
                "ndmi_p25": get_percentile(
                    percentiles,
                    25,
                ),
                "ndmi_median": get_percentile(
                    percentiles,
                    50,
                ),
                "ndmi_p75": get_percentile(
                    percentiles,
                    75,
                ),
                "ndmi_p90": get_percentile(
                    percentiles,
                    90,
                ),
...
def add_trend_columns(
    ...
    dataframe["ndmi_mean"] = pd.to_numeric(
        dataframe.get("ndmi_mean"),
        errors="coerce",
    )
    ...
    ) & dataframe["ndmi_mean"].notna()

    dataframe[
        "ndmi_change_from_previous_usable"
    ] = pd.NA

    dataframe["ndmi_rolling_mean_3"] = pd.NA
...
def create_plot(
    ...
    """Crée une courbe temporelle du NDMI."""
    ...
    axis.set_title(
        f"Évolution du NDMI — {parcel_name}"
    )

    axis.set_xlabel("Date d'acquisition")
    axis.set_ylabel("NDMI")
    axis.set_ylim(-1, 1)
    axis.grid(True)
    axis.legend()
...
def main() -> None:
    ...
    print("Calcul des statistiques NDMI...")
    ...
    csv_file = (
        PROCESSED_DIR
        / f"ndmi_timeseries_{SELECTED_SITE_ID}.csv"
    )
    ...
    statistics_json_file = (
        PROCESSED_DIR
        / f"ndmi_statistics_{SELECTED_SITE_ID}_raw.json"
    )

    plot_file = (
        OUTPUT_DIR
        / f"ndmi_timeseries_{SELECTED_SITE_ID}.png"
    )
